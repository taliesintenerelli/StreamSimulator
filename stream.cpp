...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Handle ParserError exception
        cout << e.what() << endl; // Print the what() message in ParserError
        return 1;
    } catch (const std::exception& e) {
        // Handle other exceptions
        cout << parser_except_msg << endl; // Output the string defined in `parser_except_msg`
        return 1;
    }
    ...
    while(option != 0) {
        ...
        }   catch (const UserNotLoggedInError& e) {
              cout << no_user_except_msg << endl; 
              continue; 
          } catch (const RatingLimitError& e) {
              cout << rating_limit_except_msg << endl; 
          } catch (const ReviewRangeError& e) {
